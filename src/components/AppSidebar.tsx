...
import { Home, Trophy, Users, MessageCircle, User, Plus, Crown, Settings } from "lucide-react";
...
export function AppSidebar() {
  ...
  return <Sidebar className="border-r border-purple-500/30 bg-gradient-to-b from-gray-900 to-purple-900/50">
      <SidebarContent>
        {/* Main Navigation */}
        <SidebarGroup>
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {tournamentItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    <NavLink to={item.url} className={`${getNavClass(item.url)} transition-all duration-200 flex items-center space-x-3 px-3 py-2 rounded-lg`}>
                      <item.icon className="h-5 w-5 flex-shrink-0" />
                      {!collapsed && <span className="font-medium text-gray-950">{item.title}</span>}
